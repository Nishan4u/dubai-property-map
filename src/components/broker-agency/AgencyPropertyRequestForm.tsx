...
export function AgencyPropertyRequestForm({
  projectId,
  salespersons,
}: {
  projectId: string;
    ...
    return (
      <div className="rounded-xl border border-emerald-600/40 bg-emerald-500/10 p-5 text-center">
        ...
        <p className="mt-1 text-xs text-ink-400">
          ...
          notified and will contact your agency directly.
        </p>
      </div>
    );
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await fetch("/api/broker-agency/property-requests", {
      method: "POST",
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3 rounded-xl border border-navy-700 bg-navy-850 p-5">
      <p className="flex items-center gap-2 text-sm font-semibold text-ink-100">
        <Send className="h-4 w-4 text-gold-400" /> Request Property (Agency)
      </p>
