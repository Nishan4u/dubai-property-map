...
export function BrokerClientsTable({ brokerId, clients }: { brokerId: string; clients: CrmClientRow[] }) {
        ...
        columns={[
          {
            header: "Name",
            render: (c) => (
              <Link href={`/broker/clients/${c.id}`} className="font-medium text-ink-100 hover:text-gold-400">
                {c.full_name}
              </Link>
            ),
          },
