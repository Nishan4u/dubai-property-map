...
// in this codebase.
export function SalespersonClientsTable({ salespersonId, clients }: { salespersonId: string; clients: CrmClientRow[] }) {
        ...
        columns={[
          {
            header: "Name",
            render: (c) => (
              <Link href={`/salesperson/clients/${c.id}`} className="font-medium text-ink-100 hover:text-gold-400">
                {c.full_name}
              </Link>
            ),
          },
