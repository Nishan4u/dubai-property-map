...
export function BrokerListingCard({
  ...
  return (
    ...
    >
      ...
      <div className="p-4">
        <p className="truncate font-serif text-sm font-medium text-ink-100">{listing.title}</p>
        <p className="text-xs text-ink-500">
          {listing.property_type} · <span className="capitalize">{listing.listing_type}</span> · {listing.communities?.name ?? "Dubai"}
        </p>
        <p className="mt-1 font-serif text-sm italic font-semibold text-gold-400">{formatAed(listing.price_aed)}</p>
