...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: reservation } = await supabase
    ...
    .select(
      "id, status, unit_number, price_aed, deposit_amount_aed, sign_token, crm_clients(full_name, email, phone), projects(name, developers(name), communities(name)), project_unit_types(unit_name, unit_type)"
    )
    .eq("id", id)
    .maybeSingle();
