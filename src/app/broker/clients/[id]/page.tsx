...
import { getCrmClientDetailForBroker, getPublishedProjects, requireBrokerProfile } from "@/lib/supabase/queries";
...
export default async function BrokerClientDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [detail, projectRows] = await Promise.all([
    getCrmClientDetailForBroker(id, profile.broker_id),
